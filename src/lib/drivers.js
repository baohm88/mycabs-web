...
export async function applyToOpening({ companyId }) {
    const res = await api.post("/api/drivers/apply", { companyId });
...
export async function respondInvitation(inviteId, accept) {
    ...
    return (
        await api.post(`/api/drivers/invitations/${inviteId}/respond`, {
            accept,
        })
    ).data?.data;
}
...
export async function getMyTransactions(params) {
    return (await api.get("/api/drivers/me/transactions", { params })).data
        ?.data;
}

export async function getMyApplications(params) {
    return (await api.get("/api/drivers/me/applications", { params })).data
        ?.data;
}

export async function getMyInvitations(params) {
    return (await api.get("/api/drivers/me/invitations", { params })).data
        ?.data;
}
